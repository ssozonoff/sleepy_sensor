...
void SensorMesh::handleCommand(uint32_t sender_timestamp, char* command, char* reply) {
  ...
  } else if (memcmp(command, "zone ", 5) == 0) {  // zone management commands
    const char* subcmd = &command[5];

    if (memcmp(subcmd, "set ", 4) == 0) {
      // zone set <name>
      const char* zone_name_arg = &subcmd[4];
      if (strlen(zone_name_arg) == 0) {
        strcpy(reply, "Err - zone name required");
      } else if (strlen(zone_name_arg) >= 32) {
        strcpy(reply, "Err - zone name too long (max 31 chars)");
      } else {
        setBroadcastZone(zone_name_arg);
        sprintf(reply, "Zone set: %s", zone_name_arg);
      }
    } else if (strcmp(subcmd, "clear") == 0) {
      // zone clear
      clearBroadcastZone();
      strcpy(reply, "Zone cleared (standard flood)");
    } else if (strcmp(subcmd, "status") == 0 || strcmp(subcmd, "info") == 0) {
      // zone status
      const char* current_zone = getBroadcastZoneName();
      if (current_zone) {
        sprintf(reply, "Zone: %s (transport codes enabled)", current_zone);
      } else {
        strcpy(reply, "Zone: none (standard flood)");
      }
    } else {
  ...
  } else if (memcmp(command, "advert ", 7) == 0) {  // advertisement interval commands
    const char* subcmd = &command[7];

    if (memcmp(subcmd, "set ", 4) == 0) {
      // advert set <count>
      uint32_t count = atoi(&subcmd[4]);
...
void SensorMesh::broadcastTelemetry() {
  // Read current sensor data into telemetry buffer
  telemetry.reset();
  telemetry.addVoltage(TELEM_CHANNEL_SELF, (float)board.getBattMilliVolts() / 1000.0f);

  // Query other sensors (if any)
  ...
  // Create packet with timestamp + telemetry data
 ...
 *
 * WHAT ARE TRANSPORT CODES?
 * Transport codes are cryptographic identifiers (HMAC-SHA256) attached to packets
 * that allow repeaters to selectively forward packets based on configured zones.
 *
 * HOW IT WORKS:
 * 1. Sensor sets a zone name (e.g., "building-a")
 * 2. Zone name is hashed to create a 128-bit transport key
 * 3. For each packet, a 16-bit transport code is calculated: HMAC(key, payload)
 * 4. Two codes are attached to each packet (codes[0] = zone, codes[1] = sensor-specific)
 * 5. Repeaters calculate codes using their stored zone keys
 * 6. If codes match → packet forwarded; if no match → packet dropped
 *
 * BACKWARD COMPATIBILITY:
 * - If zone is NULL/empty, uses standard ROUTE_TYPE_FLOOD (no transport codes)
 * - All repeaters will forward standard flood packets (via wildcard region)
 * - If zone is set, uses ROUTE_TYPE_TRANSPORT_FLOOD (+4 bytes overhead)
 * - Only repeaters with matching region keys will forward
 *
 * BENEFITS:
 * - Reduces network congestion (packets only forwarded in relevant zones)
 * - Better scalability for large deployments
 * - Geographic/logical isolation of sensor networks
 * - Lower battery consumption (fewer packet collisions)
 *
 * CONFIGURATION:
 * - Default zone: DEFAULT_BROADCAST_ZONE (defined in SensorMesh.h)
 * - Runtime control: "zone set <name>", "zone clear", "zone status"
 * - Compile-time override: -D DEFAULT_BROADCAST_ZONE=\"my-zone\"
 *
 * ============================================================================== */
...
void SensorMesh::setBroadcastZone(const char* name) {
  ...
  // Store zone name
  strncpy(zone_name, name, sizeof(zone_name) - 1);
  zone_name[sizeof(zone_name) - 1] = 0;

  // Calculate transport key from zone name using SHA256
  SHA256 sha;
  sha.reset();
...
void SensorMesh::clearBroadcastZone() {
  zone_name[0] = 0;
...
const char* SensorMesh::getBroadcastZoneName() const {
  return zone_name[0] ? zone_name : NULL;
}
